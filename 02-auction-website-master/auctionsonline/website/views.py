...
def index(request):
    ...
    # auc_counts = Auction.objects.filter(time_ending__gte=datetime.now()).order_by('time_starting').all().count()
...
def list_users(request, page=1):
    ...
    paginator = Paginator(auctions, 10)  # 5 users per page
...
def raise_bid(request, auction_id):
    ...
    try:
        if request.session['username']:
            user = User.objects.get(username=request.session['username'])
            userDetails = UserDetails.objects.filter(user_id=user.id).first()
...
def login_page(request):
    ...
    # return index(request)
    return list_users(request, page=1)
